...
export const getBufToken = async (token?: string | Secret) => {
  if (env.get("BUF_TOKEN")) {
    return dag.setSecret("BUF_TOKEN", env.get("BUF_TOKEN")!);
  }
  if (token && typeof token === "string") {
    try {
      ...
      return dag.setSecret("BUF_TOKEN", token);
    }
  }
